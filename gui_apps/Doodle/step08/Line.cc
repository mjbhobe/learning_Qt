...
void Line::saveToStream(QDataStream& ds) const
{
  ...
  ds << qint32(_penWidth);
  qDebug() << QString("Line::saveToStream() - penWidth = %1").
    arg(_penWidth);
  ...
  // save points
  ds << _points->size();
  qDebug() << QString("Line::saveToStream() - saving %1 points")
    .arg(_points->size());
  foreach(QPoint pt, *_points)
...
void Line::loadFromStream(QDataStream& ds)
{
  ...
  ds >> penWidth;
  _penWidth = penWidth;
  qDebug() << QString("Line::loadFromStream() - penWidth = %1")
    .arg(_penWidth);
  ...
  qint32 numPoints;
  ds >> numPoints;
  qDebug() << QString("Line::loadFromStream() - expecting %1 points")
    .arg(numPoints);
  ...
  delete _points;
  _points = points;
  qDebug() << QString("Line::loadFromStream() - loaded %1 points")
    .arg(_points->size());
}
...
QDataStream& operator >> (QDataStream& ds, Line& line)
{
  ...
  if (line._points != 0) delete line._points;
  line._points = points;
  return ds;
}
