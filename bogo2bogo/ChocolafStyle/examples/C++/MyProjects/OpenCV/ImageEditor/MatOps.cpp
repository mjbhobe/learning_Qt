...
MatOp::MatOp(const QPixmap &pixmap, QObject *parent /*= nullptr*/) : QObject(parent)
{
   ...
   m_srcMat = cv::Mat(m_image.height(), m_image.width(), CV_8UC3, m_image.bits(), m_image.bytesPerLine());
}

QPixmap MatOp::blur(cv::Size ksize /*= cv::Size(8,8)*/, cv::Point anchor /*=cv::Point(-1,-1)*/,
                    int borderType /*=4*/)
{
   cv::Mat conv;
...
QPixmap MatOp::sharpen(int intensity /*= 2*/, cv::Size ksize /*= cv::Size(9, 9)*/, double sigmaX /*= 0.0*/,
...
QPixmap MatOp::erode(cv::Point anchor /*= cv::Point(-1, -1)*/, int iterations /*= 1*/, int borderType /*= 0*/,
                     const cv::Scalar &borderValue /*= cv::morphologyDefaultBorderValue()*/)
{
   cv::erode(m_srcMat, m_srcMat, cv::Mat(), anchor, iterations, borderType, borderValue);
   QImage erodeImage(m_srcMat.data, m_srcMat.cols, m_srcMat.rows, m_srcMat.step, QImage::Format_RGB888);
