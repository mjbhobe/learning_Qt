...
{
  ost << "Employee: name - " << emp.name() << " age - " << emp.age() << " salary - "
      << emp.salary() << endl;
  return ost;
}
...
int main(void)
{
  ...
  // double the salary
  std::transform(emps.begin(), emps.end(), emps.begin(), [](Employee &emp) -> Employee {
    return Employee(emp.name(), emp.age(), emp.salary() * 2);
  });
  display_vector(emps);
